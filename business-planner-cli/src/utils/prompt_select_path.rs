...
fn get_valid_paths(
    ...
    let mut valid_dir_entries = files
        .filter_map(|file| {
            ...
            if path.is_file()
                && let PathFilter::File(extension) = *path_filter
            {
                match extension {
                    Some(extension) => match path.extension()?.to_str()?.eq(extension) {
...
pub async fn prompt_select_path(
    path: &Path,
    ...
    while file_path.is_none() {
        ...
        let valid_path_names = valid_paths
            .iter()
