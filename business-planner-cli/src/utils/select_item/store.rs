...
use business_planner::api::{item::StoreItem, session::Session};
use clap::{Arg, ArgMatches, builder::{StringValueParser, TypedValueParser}};
use uuid::Uuid;

use crate::Error;

static ID_ARG_ID: &str = "store_id";
static ID_ARG_KEBAB: &str = "store-id";
static NAME_ARG_ID: &str = "store_name";
static NAME_ARG_KEBAB: &str = "store-name";
...
impl StoreArg {
    ...
    pub fn parse_arg_matches(arg_matches: &ArgMatches, session: &Session) -> Result<Uuid, Error> {
        ...
        let mut candidates = session.get_by_name::<StoreItem>(name);

        match (candidates.len()).cmp(&1) {
            Ordering::Less => Err(Error::NotFound(format!("store with name {}", name))),
            Ordering::Equal => Ok(*candidates.remove(0)),
            Ordering::Greater => Err(Error::MultipleFound(format!("store with name {}", name))),
        }
    }
}