...
fn list_directory_contents(
    path: &PathBuf,
    filter: Option<DirectoryEntriesFilter>,
) -> Result<Vec<PathBuf>, error::PluginManagementError> {
...
pub fn get_plugins() -> Result<HashMap<String, Plugin>, error::PluginManagementError> {
    ...
    let directory_contents =
        list_directory_contents(&current_directory, Some(DirectoryEntriesFilter::Directory))?;

    let mut plugins = HashMap::new();

    directory_contents.into_iter().try_for_each(|directory| {
        let Ok(files) = list_directory_contents(&directory, Some(DirectoryEntriesFilter::File))
        else {
        ...
        let plugin_directory_name = directory
            .file_name()
            .ok_or(PluginManagementError::ReadDirectoryError)?;
...
impl PluginResponses {
    pub fn try_next(&mut self) -> Option<Result<PluginResponse<'_>, Error>> {
        ...
        let plugin_response = match message {
            Message::DataRequest => PluginResponse::AnyDataRequest(AnyDataRequest {
...
impl PluginProcess {
    fn new(mut child_process: Child) -> PluginProcess {
        ...
        let responses = PluginResponses { stdin, stdout };
...
pub fn remove_plugin(plugin_name: &str) -> Result<(), Error> {
    ...
    if let Err(error) = trash::delete(plugin.path) {
        Err(PluginManagementError::TrashError(error))?;
    }
    Ok(())
}
