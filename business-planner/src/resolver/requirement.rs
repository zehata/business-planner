...
impl ItemResolver for RequirementResolver {
    ...
    fn resolve<'a>(
        ...
        id: &Uuid,
        _registry_item: &Self::ItemObject,
    ) -> Result<&'a Self::Data, ReadError> {
        let resolver: Result<&mut RequirementResolver, ReadError> =
            match resolver.requirements.entry(*id) {
                Entry::Occupied(entry) => Ok(entry.into_mut()),
                Entry::Vacant(entry) => {
                    let data = RequirementData::new();
                    Ok(entry.insert(RequirementResolver { data }))
                }
            };
        Ok(&resolver?.data)
    }
