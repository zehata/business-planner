...
use crate::{
    ...
    io::{
        error::ReadError,
        excel::{create_vec_from_cells, read_once},
    },
    item::{DataSource, StoreItem},
    ...
    fn resolve<'a>(
        ...
        let resolver: Result<&mut StoreResolver, ReadError> = match resolver.stores.entry(*id) {
            ...
            Entry::Vacant(entry) => {
                    ...
                    Some(DataSource::Excel(excel_data_source)) => {
                        ...
                        match (path, sheet, range) {
                            (Some(path), Some(sheet), Some(range)) => {
                                let range = read_once(path, sheet, range)?;
                ...
                Ok(entry.insert(StoreResolver { data }))
            }
        };
        Ok(&resolver?.data)
    }
